...
import openpyxl
from import_app import err_model
            ...
            for i in range(2, table.max_row+1):
                ...
                if date is not None:
                    ...
                    if ':' not in date:
                        try:
                            ...
                            day = time.date().day
                            print('e1', year, month, day)
                            # 检验年月日不为0
                            tmp = 1/year + 1/month + 1/day
                            rowDic.update({'dataTime': err_model.dataModel(date, 1)})
                        except Exception as e:
                            print(str(e))
                            rowDic.update({'dataTime': err_model.dataModel(table.cell(i, 1).value, 2)})
                            flagLocal = 0
                            flagGlobal = 0
                    else:
                        try:
                    ...
                    rowDic.update({'dataTime': err_model.dataModel("", 2)})
                    flagLocal = 0
                    flagGlobal = 0
                ...
                for index in range(2, 5):
                    ...
                    if rowName and type(rowName) == str and len(rowName) <= 127:
                        ...
                        if rowName is None:
                            rowDic.update({names[index - 2]: err_model.dataModel("", 2)})
                        else:
            ...
            for i in range(6, table.max_row):
                ...
                for index in range(4, 7):
                    ...
                    if rowName is not None:
                        rowName = str(rowName)
                        if rowName.isdigit() and len(rowName) <= 20:
                        ...
                        rowDic.update({names[index - 4]: err_model.dataModel('', 2)})
                        flagGlobal = 0
                        flagLocal = 0
