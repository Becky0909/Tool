...
from import_app import exam_file
from import_app import importToDB
from chart_app import result_models,query_operate
...
def importHW(queryType, subPage, subSize, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        book = openpyxl.load_workbook("./sql_app/TemporaryFile.xlsx")
    except Exception as e:
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Sheet1'
        wb.save('./sql_app/TemporaryFile.xlsx')
        book = openpyxl.load_workbook("./sql_app/TemporaryFile.xlsx")
    try:
        # excel内容写入TemporaryFile文件以便解析
        res = file.file.read()
        with open("./sql_app/TemporaryFile.xlsx", "wb") as f:
            f.write(res)
            f.close()
        # 文件列数校验
        if not exam_file.rowExam(queryType):
    ...
    if flag == 0:
        ...
        return schemas.ResponseStandard("您上传的文件，部分数据出现错误，请检查后重新上传", 402,
...
def exModel(queryType, db: Session = Depends(get_db)):
    filedic = {1: 'TotalClickHuaweiModel.xlsx', 2: 'ServiceNumberHuaweiModel.xlsx', 3: 'TableXiaomiModel.xlsx'}
    nameDic = {1:'华为运营数据总点击量报表',2:'华为运营数据报表',3:'小米运营数据报表'}
    short = '/' + 'app/export_model' + '/'
    filePath = os.path.abspath(os.path.join(os.path.dirname("__file__"),
                                            os.path.pardir)) + short + filedic[int(queryType)]
    response = FileResponse(filePath,filename=nameDic[int(queryType)])
    return response
